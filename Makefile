...
PROJECT ?= github.com/zawiw/traefik-helm-chart
################################## Functionnal targets
...
lint: lint-requirements
	@echo "== Linting Chart..."
	@git remote add traefik https://github.com/zawiw/traefik-helm-chart >/dev/null 2>&1 || true
...
deploy: global-requirements $(DIST_DIR) $(HELM_REPO)
	...
	@helm repo index --merge $(HELM_REPO)/index.yaml --url https://zawiw.github.io/traefik-helm-chart/ $(HELM_REPO)
